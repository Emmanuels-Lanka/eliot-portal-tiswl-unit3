...
const RoamingQcReportPage = () => {
  ...
  const processData = (rawData: RawDataType[]): ProcessedDataType => {
    const groupedByOperation = rawData.reduce<Record<string, RawDataType[]>>(
      (acc, item) => {
    ...
    return {
      ...
      data: Object.entries(groupedByOperation).map(
        ([obbOperationId, records]) => ({
          obbOperationId,
          records,
        })
      ),
  ...
  const handleProcessReportData = async (data: {
    unit: string;
    lineId: string;
    date: Date;
  }) => {
    try {
      ...
      for (const obbSheet of obbSheets) {
        const rawData: RawDataType[] = await fetchRoamingQcData(
          obbSheet.id,
          formattedDate
        );
  ...
  const generatePdfReport = (reportData: ReportDataType[]) => {
    return (
      <PDFDownloadLink
        document={<RoamingQcReportTemplate data={reportData} />}
  ...
  return (
    <div className="mx-auto max-w-full pl-6 pr-6 ">
      <SelectUnitLineAndDate handleSubmit={handleProcessReportData} />
      ...
      {reportData.length > 0 && (
        <div className="mt-8 p-8 bg-slate-100 rounded-lg border flex flex-col items-end gap-4">
          ...
          <div className="w-full pdf-viewer">
            <RoamingQcReportViewer data={reportData} />
          </div>
        </div>
      )}
    </div>
