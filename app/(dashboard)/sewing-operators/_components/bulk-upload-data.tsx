...
const schemaKeys = ["name", "employeeId", "rfid", "gender", "designation"];
...
const BulkUploadSewingOperatorData = () => {
  ...
  const handleSaveData = async () => {
    ...
    if (isValid && jsonData) {
      try {
        ...
        await axios.post("/api/sewing-operator/bulk", dataToUpload, {
      ...
      {previewData ? (
        ...
        <p className="text-sm text-slate-500">
          {file
            ...
            : "Please select an Excel file to upload sewing operator data."}
        </p>
      )}
...
export default BulkUploadSewingOperatorData;
