...
const AnalyticsChart = ({ obbSheets }: AnalyticsChartProps) => {
  ...
  const [heatmapData, setHeatmapData] =
    useState<RoamingQcChartFunctionOutputTypes | null>(null);

  function processRoamingQcData(
    productionData: RoamingQcDataTypes[]
  ): RoamingQcChartFunctionOutputTypes {
    const hourGroups = [
      "7:00 AM - 8:00 AM",
      "8:00 AM - 9:00 AM",
      "9:00 AM - 10:00 AM",
    ...
    const resultData = hourGroups.map((hourGroup) => ({
      hourGroup,
      operation: Object.values(operationsMap)
        .filter((data) => getHourGroup(data[0].timestamp) === hourGroup)
        .map((group) => {
    ...
    const categories = Array.from(
      new Set(resultData.flatMap((hg) => hg.operation.map((op) => op.name)))
    );
  ...
  const handleFetchProductions = async (data: {
    ...
    try {
      ...
      const response = await axios.get(
        `/api/roaming-qc?obbSheetId=${data.obbSheetId}&date=${formattedDate}`
      );
      ...
      <div className="mx-auto max-w-[1680px]">
        {heatmapData ? (
          <div className="mt-12">
            ...
            <RoamingQcHeatmap heatmapData={heatmapData} />
          </div>
        ) : (
