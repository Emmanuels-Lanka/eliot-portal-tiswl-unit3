...
const SelectObbSheetAndDate = ({
    ...
    return (
        <div className='mt-6 border px-6 pt-6 pb-10 shadow-md rounded-lg bg-slate-100'>
    ...
    <PopoverTrigger asChild>
        ...
        >
            {field.value
                ? timeValues.find((time) => time.key.toString() === field.value)?.value
                : "Select Time slot..."}
            <ChevronsUpDown className="ml-2 h-4 w-4 shrink-0 opacity-50" />
        </Button>
    </PopoverTrigger>
    <PopoverContent className="p-0">
        <Command>
            <CommandList>
                <CommandEmpty>No Time Slots</CommandEmpty>
                <CommandGroup>
                    {timeValues.map((time) => (
                        <CommandItem
                            key={time.key}
                            value={time.value}
                            onSelect={() => {
                                form.setValue("timeValue", time.key.toString()); 
