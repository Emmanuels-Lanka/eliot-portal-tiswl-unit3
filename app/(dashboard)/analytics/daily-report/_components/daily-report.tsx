...
const ReportTable = ({ obbSheets }: AnalyticsChartProps) => {
  ...
  const calculateAverageEfficiency = (groupedData: {
    [key: string]: ReportDataOut[];
  }) => {
    return Object.values(groupedData).map((operatorData) => {
      // Get all valid timestamps for the operator
      const allTimestamps = operatorData
        .flatMap((op) => [
      ...
      const efficiency = Math.max(
        ...
        Math.min(
          100,
          Math.round((totalEarnedMinutes / actualWorkingMinutes) * 100)
        )
      );
  ...
  const mergeEfficiencyData = (
    achievements: any[],
    effData: ProductionEfficiency[]
  ) => {
    return achievements.map((achievement) => {
      // Find matching efficiency data by rfid
      const matchedEfficiency = effData.find(
        (eff) => eff.operatorRfid === achievement.rfid
      );

      return {
        ...achievement,
        efficiencyDB: matchedEfficiency
          ? matchedEfficiency.efficiency
          : achievement.efficiency, // Update efficiency if found
      };
    });
  };
    ...
    try {
      ...
      while (position < canvas.height) {
        ...
        pdf.addImage(
          ...
          10,
          imgWidth,
          (canvasSlice.height * imgWidth) / canvas.width
        );
  ...
  return (
    <div className="mx-auto max-w-full pl-6 pr-6 ">
      ...
      {date.length > 0 && (
        <div className="bg-slate-50 w-full border rounded-lg shadow-sm pl-4 pr-4 mt-10">
          <CardHeader>
            <CardTitle>
            ...
            <Table className="text-sm min-w-full border-collapse border border-gray-300">
              <TableHeader>
                <TableRow className="h-8">
                  {" "}
                  {/* Reduce row height */}
                  <TableHead className="px-2 py-1">Seq No</TableHead>{" "}
                  {/* Reduce padding */}
              ...
              <TableBody>
                {data.map((d, rid) => (
                  <TableRow key={rid} className="h-8">
                    {" "}
                    {/* Reduce row height */}
                    <TableCell className="px-2 py-2">{d.seqNo}</TableCell>{" "}
                    {/* Reduce padding */}
