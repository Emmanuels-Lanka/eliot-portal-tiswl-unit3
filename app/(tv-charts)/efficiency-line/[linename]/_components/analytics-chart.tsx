...
import { getLine, getLinebyOS, getObbSheetID } from "@/components/tv-charts/achievement-rate-operation/actions";
...
const     AnalyticsChart = ({ linename }: { linename: string }) => {
  ...
  const fetchObbSheetId = async () => {
    ...
    //   const id = await getObbSheetID(linename);
      const line =await getLine(linename);
      if (line) {
...
const abbreviatePart = (part: string) => {
  ...
  const FirstName =(txt :string)=>{
    ...
    if (minutes >= 5) {
        ...
        const categories = operations.map(op => ` ${FirstName(op.operator.operator.name)} - ${shortenOperationName(op.obbOperation.operation.name)} - ${op.obbOperation.smv} - ${abbreviatePart(op.obbOperation.part)} -  ${op.obbOperation?.sewingMachine?.machineId || 'Unknown Machine ID'}  - ${op.obbOperation.seqNo}`);
        const part = operations.map(op=>(op.obbOperation.part.toLowerCase()))
      ...
      <>
        <div className="h-full w-full flex flex-col justify-center items-center  ">
          <div className="h-auto ">
            <div className="flex justify-center items-center gap-1 w-screen bg-white bg-opacity-10 backdrop-filter backdrop-blur-lg border border-white border-opacity-20 rounded-lg shadow-lg">
                <div>
              ...
              <h1 className="text-[#0071c1]  text-3xl  text-center">
                Dashboard - Efficiency TV Graph- {lineName}{" "}
              </h1>
            </div>
