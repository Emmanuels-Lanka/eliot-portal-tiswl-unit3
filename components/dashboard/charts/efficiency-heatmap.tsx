...
const EffiencyHeatmap = ({
  ...
  let series: { name: string; data: (number | null)[] }[] =
    heatmapData.data.map((hourGroup) => ({
      name: hourGroup.hourGroup,
      data: hourGroup.operation.map((op) =>
        op.efficiency === null ? -1 : op.efficiency
      ),
    }));
  ...
  const saveAsPDF = async () => {
    if (chartRef.current) {
      ...
      img.onload = () => {
        ...
        pdf.setFontSize(30);
        pdf.text(
          "Dashboard - Operation Efficiency (60) ",
          logoX + logoWidth + 10,
          83,
          { align: "left" }
        );
  ...
  const chartWidth =
    heatmapData && heatmapData.categories.length > 0
      ? heatmapData.categories.length * 100
      : 1000;
  ...
  const getChartWidth = () => {
    ...
    const calculatedWidth =
      heatmapData.categories.length * baseWidthPerCategory;
